...
class CircosView extends React.Component<OverViewProps, CircosViewState>
  ...
  drawCircos(features: any, chroms: any) {
    ...
    var chordMatrix = new Array(3);
    for(let y = 0; y < chromsLen.length; y++) {
      chordMatrix[y] = new Array(3);
      for(let x = 0; x < chromsLen.length; x++) {
        chordMatrix[y][x] = 0;
        if (y === x) {
          chordMatrix[y][x] = chromsLen[x];
        }
      }
    }
    

    //var chordMatrix = math.diag(chromsLen).toArray();
